...
class OKX():
    ...
    def data(self, api_key, secret_key, passphras, request_path="/api/v5/account/balance?ccy=USDT", body='', meth="GET"):
    ...
    async def get_balance(self, session, account, api_key, secret_key, passphras, token, balances):

        _, request_path, headers = self.data(api_key, secret_key, passphras)
        url = "https://www.okx.cab" + request_path 

        async with session.get(url, ssl=False, timeout=10, headers=headers) as resp:

            # spot balance
            _, _, headers   = self.data(api_key, secret_key, passphras, request_path=f"/api/v5/account/balance?ccy={token}")
            ...
            _, _, headers   = self.data(api_key, secret_key, passphras, request_path=f"/api/v5/asset/balances?ccy={token}", meth="GET")
    ...
    async def trade(self, session, account, api_key, secret_key, passphras, proxy, token_sell, token_buy, amount, all_balance, price, is_market_price, spread, min_price, breaker, min_sell, cancel_order, cl_order_time):
        while True:
            try:
                _, request_path, headers = self.data(api_key, secret_key, passphras)
                url = "https://www.okx.cab" + request_path 
                ...
                async with session.get(url, ssl=False, timeout=10, headers=headers) as resp:
                    ...
                    try:
                        ...
                        _, _, headers = self.data(api_key, secret_key, passphras, request_path=f"/api/v5/asset/balances?ccy={token_sell}", meth="GET")
                        ...
                        if float(funding_balance) > 0:
                            ...
                            _, _, headers = self.data(api_key, secret_key, passphras, request_path=f"/api/v5/asset/transfer", body=str(body), meth="POST")
                    ...
                    # check balance
                    _, _, headers = self.data(api_key, secret_key, passphras, request_path=f"/api/v5/account/balance?ccy={token_sell}")
                                ...
                                if balance_of_coin >= min_sell:
                                    ...
                                    if float(price_to_order) >= min_price:
                                        ...
                                        if token_sell == 'USDT':
                                            ...
                                            _, _, headers = self.data(api_key, secret_key, passphras, request_path=f"/api/v5/trade/order", body=str(body), meth="POST")
                            ...
                            if (balance_of_coin >= min_sell and float(price_to_order) >= min_price):
                                if int(order['code']) == 0:
                                    try:
                                        ...
                                        _, _, headers = self.data(api_key, secret_key, passphras, request_path=f"/api/v5/trade/order?ordId={order_id}&instId={pair_}")
                                        ...
                                        if order_filled_size > 0:
                                            ...
                                            try:
                                                ...
                                                if cancel_order:
                                                    ...
                                                    _, _, headers = self.data(api_key, secret_key, passphras, request_path=f"/api/v5/trade/cancel-order", body=str(body), meth="POST")
    ...
    async def main(self, data_file_name='', token_sell='', token_buy='', amount='', all_balance='', price='', is_market_price='', spread='', min_price='', breaker='', min_sell='', cancel_order='', cl_order_time='', module='', balance_token='', balances=''):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for data in exchanges_data:
                ...
                if exchange == 'okx':
                    ...
                    if module == 0:
                        task = asyncio.create_task(self.get_balance(session, account, api_key, api_secret, password, balance_token, balances))
                    if module == 1:
                        task = asyncio.create_task(self.trade(session, account, api_key, api_secret, password, proxy, token_sell, token_buy, amount, all_balance, price, is_market_price, spread, min_price, breaker, min_sell, cancel_order, cl_order_time))
